...
## CreatorFlow Specific Coding Rules

- **Database Path**: Always use the constant `DB_PATH = "creatorflow.db"` to connect to the SQLite database.
- **Do not modify seeded data**: Do not alter database seeding logic in `app/database.py`.
- **Model Constraints**:
  - `pipeline_coordinator` must run on `gemini-2.5-flash`.
  - `compliance_auditor` must run on `gemini-2.5-pro` for high-reasoning contract checks.
- **Security Check**:
  - The security guardrail callback in `app/security.py` must always intercept user inputs and scrub PII or intercept prompt injection overrides.
